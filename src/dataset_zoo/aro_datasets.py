...
class POPE(Dataset):
    def __init__(self, image_preprocess=None, root_dir='/data3/KJE/code/WIL_DeepLearningProject_2/VLM_Hallu/data', max_words=30, split="coco",
                 type = 'adversarial',image_perturb_fn=None, download=False):  

        self.root_dir = os.path.join(root_dir,'POPE',split)
        ...
        if split == 'coco' or split == 'aokvqa':
            self.image_root = os.path.join(root_dir,'COCO/images/val2014')

        elif split == 'gqa':
            self.image_root = os.path.join(root_dir,'GQA/images/images')

        self.annotation = load_jsonl(os.path.join(self.root_dir,f'{split}_pope_{type}.json'))
        ...
        for img_id, ann in tqdm(enumerate(self.annotation)):
            ...
            test_case['question_id'] = ann['question_id']
            test_case["image"] = ann['image']
            test_case['caption'] = ann['text']
    ...
    def __getitem__(self, index):  
        ...
        item = edict({"question_id":[test_case["question_id"]],"image_options": [image], "caption_options": [test_case["caption"]],"labels":[test_case["label"]]})
...
def get_pope(image_preprocess, image_perturb_fn, text_perturb_fn, max_words=30, root_dir='/data3/KJE/code/WIL_DeepLearningProject_2/VLM_Hallu/data', split="coco",type="random"):
    return POPE(root_dir=root_dir, split=split, image_preprocess=image_preprocess, image_perturb_fn=image_perturb_fn, max_words=max_words, type=type)
