...
def config():
    ...
    parser.add_argument("--model-name", default="llava1.5", type=str, \
            choices=[ "llava1.5","llava1.6","instruct_blip"])
    parser.add_argument("--dataset", default="Controlled_Images_A", type=str, \
            choices=[ "Controlled_Images_A", "Controlled_Images_B", \
            "COCO_QA_one_obj", "COCO_QA_two_obj", "VG_QA_one_obj", "VG_QA_two_obj", "VSR","POPE"])
    
    ##POPE!! 
    parser.add_argument("--type", type=str, choices=[ "adversarial", "popular", "random"])
    parser.add_argument("--split", type=str, choices=[ "coco", "aokvqa", "gqa"])
...
def main(args):
    seed_all(args.seed) 
    model, image_preprocess = get_model(args.model_name, args.device, args.method, root_dir='/data3/hg_weight/hg_weight')
    
    if args.dataset =='POPE':
        dataset = get_dataset(args.dataset, image_preprocess=image_preprocess,type=args.type,split=args.split)
    ...
    if args.dataset=='VSR':
        ...
        elif args.method=='fuzzysteering':  
            scores, correct_id = model.fuzzy_get_out_scores_wh_batched(args.dataset,joint_loader,args.method,args.weight,args.option,args.threshold,args.weight1,args.weight2)
        ...
        #self, dataset, joint_loader, method, weight, split, type\

        if args.method=='adapt_vis': #dataset, joint_loader, method, weight, split, type, threshold, weight1, weight2)
            scores, correct_id = model.get_out_scores_pope_batched(args.dataset,joint_loader,args.method,args.weight,args.split,args.type,args.threshold,args.weight1,args.weight2)

        elif args.method=='fuzzysteering':  
            scores = model.fuzzy_get_out_scores_pope_batched(args.dataset,joint_loader,args.method,args.weight,args.split,args.type)
